...
@section('breadcrumb')
    @php
        $links = [
            ...
            ['name' => 'Manajemen Konsultasi', 'url' => route('consultation.index')],
        ];
    @endphp
    <x-breadcrumb :links="$links" title="Manajemen Konsultasi" />
@endsection
...
<div class="container-xl wide-xl">
    <div class="nk-content-body">
        ...
        <div class="nk-block">
            <div class="row g-gs">
                <table id="consultationDataTable" class="table table-bordered" style="width: 100%;">
                    <thead class="table-light">
                        <tr>
                            <th>No</th>
                            <th>Tipe Kategori Bencana</th>
                            <th>Tipe</th>
                            <th>Video</th>
                            <th>Aksi</th>
                        </tr>
...
@push('scripts')
    <script>
        function btnDelete(_id, _name) {
            let url = '{{ route('consultation.delete', ':id') }}'.replace(':id', _id);
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            Swal.fire(
                                'Berhasil!',
                                'Data Consultation ' + _name + ' berhasil dihapus.',
                                'success'
                            ).then(() => {
                                $('#consultationDataTable').DataTable().ajax.reload(null,
                                    false);
                            });
                        },
        ...
        $(function() {
            let url = '{{ route('consultation.index') }}';
            $('#consultationDataTable').DataTable({
                ...
                columns: [{
                    ...
                    {
                        data: 'typekategori_id',
                        name: 'typekategori_id'
                    },
                    {
                        data: 'type',
                        name: 'type'
