@props(['i', 'tooth_location' => null])

@php
    use Illuminate\Support\Facades\asset;
    $terminology = collect($tooth_location)->where('display_id', $i)->first();
    $terminology_id = $terminology ? $terminology['id'] : null;
@endphp

<div id="P{{ $i }}" data-terminology-id="{{ $terminology_id }}" data-modal-target="default-modal"
    data-modal-toggle="default-modal" class="relative mb-8 tooth">
