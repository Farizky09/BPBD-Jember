...
@section('breadcrumb')
    @php
        $links = [
            ...
            ['name' => 'Tambah Korban', 'url' => ''],
        ];
    @endphp
    <x-breadcrumb :links="$links" title="Tambah Korban" class="text-center" />
@endsection
@section('main')
    <div class=" mx-auto">
        <form id="form-disasterVictims" action="{{ route('disaster_victims.store') }}" method="POST"
            enctype="multipart/form-data">
            @csrf
            <input type="hidden" name="disaster_impact_id" value="{{ $disasterImpact->id }}">

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <x-input.input label="Nama Lengkap" name="fullname" id="fullname" value="{{ old('fullname') }}"
                    type="text" required />

                <x-input.input label="NIK" name="nik" id="nik" value="{{ old('nik') }}" type="number"
                    maxlength="16" required />

                <x-input.input label="No. KK" name="kk" id="kk" value="{{ old('kk') }}" type="number"
                    maxlength="16" required />
                ...
                <x-input.input label="Usia" name="age" id="age" value="{{ old('age') }}" type="number"
                    min="0" required />
                ...
                <x-input.input label="No. Telepon" name="phone_number" id="phone_number" value="{{ old('phone_number') }}"
                    type="number" required/>

                <x-input.input label="Tempat Lahir" name="birth_place" id="birth_place" value="{{ old('birth_place') }}"
                    type="text" required />

                <x-input.input label="Tanggal Lahir" name="birth_date" id="birth_date" value="{{ old('birth_date') }}"
                    type="date" required />

                <div class="mb-4">
                    <label for="vulnerable_group" class="block text-gray-700 font-bold mb-2">Status Kelompok</label>
                    <select name="vulnerable_group" id="vulnerable_group"
                        class="select2 form-select w-full rounded border-gray-300" required>
                        <option value="general" {{ old('vulnerable_group', 'general') == 'general' ? 'selected' : '' }}>
                            Umum</option>
                        <option value="babies" {{ old('vulnerable_group') == 'babies' ? 'selected' : '' }}>Bayi/Balita
                        </option>
                        <option value="elderly" {{ old('vulnerable_group') == 'elderly' ? 'selected' : '' }}>Lansia
                        </option>
                        <option value="disabled" {{ old('vulnerable_group') == 'disabled' ? 'selected' : '' }}>Disabilitas
                        </option>
                        <option value="pregnant_women" {{ old('vulnerable_group') == 'pregnant_women' ? 'selected' : '' }}>
                            Ibu Hamil</option>
                    </select>
                </div>
        ...
        <script>
            document.getElementById('form-disasterVictims').addEventListener('submit', function(e) {
                ...
                Swal.fire({
                    ...
                    text: 'Data Korban akan disimpan',
                    icon: 'question',
