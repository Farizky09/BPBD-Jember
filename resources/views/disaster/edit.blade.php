...
@section('breadcrumb')
    ...
    @endphp
    <x-breadcrumb :links="$links" title="Edit Jenis Bencana" class="text-center" />
@endsection
<form action="{{ route('disaster.update', $disaster->id) }}" method='POST' id="form-edit">
    @csrf
    @method('PUT')
    <div class="mb-4">
        <x-input.input label="Nama Bencana" name="name" id="name" placeholder="Contoh: Banjir"
            class="w-40" required="true" value="{{ $disaster->name }}" />
