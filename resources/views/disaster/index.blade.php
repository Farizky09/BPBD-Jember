...
@section('breadcrumb')
    ...
    @endphp
    <x-breadcrumb :links="$links" title="Manajemen Jenis Bencana" class="text-center" />
@endsection
<div class="container-xl wide-xl">
    <div class="nk-content-body">
        ...
        <div class="nk-block">
            <div class="row g-gas">
                <table id="kategoriBencanaTable" class="table table-bordered" style="width: 100%">
...
@push('scripts')
    <script>
        function btnDelete(id, name) {
            let url = '{{ route('disaster.destroy', ':id') }}'.replace(':id', id);
            Swal.fire({
                title: 'apakah anda yakin?',
                text: 'Data Jenis ' + name + ' akan dihapus!',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            Swal.fire(
                                'Berhasil!',
                                'Data Jenis Bencana ' + name + ' berhasil dihapus',
                                'success'
                            ).then(() => {
                                $('#kategoriBencanaTable').DataTable().ajax.reload(null, false);
                            });
                        },
                        error: function(response) {
                            Swal.fire(
                                'Gagall!',
                                'Data Jenis Bencana' + name + 'gagal dihapus',
                                'error'
                            );
                        }
                    })
        ...
        $(function() {
            let url = '{{ route('disaster.index') }}';
            $('#kategoriBencanaTable').DataTable({
                processing: true,
                serverSide: true,
                autoWidth: false,
                responsive: true,
